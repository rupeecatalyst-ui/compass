...
assert.match(read("compass/src/config/home-loan-discovery.ts"), /home-loan-balance-transfer/);
...
assert.match(read("src/types/compass-customer-gateway.ts"), /HOME_LOAN_BT/);
assert.match(read("src/types/compass-customer-gateway.ts"), /HOME_LOAN/);
assert.match(compassIntelligenceRoute, /410|retired|gateway/i);
assert.match(compassBorrowNav, /future:\s*true/);
assert.doesNotMatch(compassBorrowNav, /ROUTES\.PERSONAL_LOAN/);
