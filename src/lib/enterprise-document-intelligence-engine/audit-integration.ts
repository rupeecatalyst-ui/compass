/**
 * EDIE audit integration — reuses Sprint 1 EAF audit infrastructure.
 */

import { appendEafAuditEntry } from "@/lib/enterprise-asset-framework";
import type { EdieAuditEntityType } from "@/types/enterprise-document-intelligence-engine";
import { getEdiePorts } from "./composition";

export function recordEdieAudit(input: {
  entityId: string;
  entityType: EdieAuditEntityType;
  action: "created" | "modified" | "lifecycle_changed" | "archived" | "uploaded" | "verified" | "approved";
  actorId: string;
  ...
  getEdiePorts().auditReferences.save({
