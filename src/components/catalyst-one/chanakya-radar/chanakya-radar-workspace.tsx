...
/**
 * CO-SPRINT-100 — CHANAKYA Radar Operational Intelligence Dashboard.
 * Replaces Matrix. Default Mission Control landing experience.
 */
export function ChanakyaRadarWorkspace() {
  ...
  const [quadrantFilter, setQuadrantFilter] = useState<ChanakyaOperationalQuadrantId | null>(
    null,
  );
  ...
  const visibleRows = useMemo(
    () => filterRowsByQuadrant(model.rows, quadrantFilter),
    [model.rows, quadrantFilter],
  );

  const tabRows = useMemo(
    () => filterRowsByActionTab(visibleRows, actionTab, scopedFiles),
    [visibleRows, actionTab, scopedFiles],
  );

  const handleQuadrantClick = useCallback((id: ChanakyaOperationalQuadrantId) => {
    setQuadrantFilter((prev) => (prev === id ? null : id));
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3 overflow-auto p-3 md:p-4">
      ...
      <div className="grid shrink-0 grid-cols-2 gap-2 lg:grid-cols-4">
        {model.kpis.map((kpi) => {
          const active = quadrantFilter === kpi.id;
          return (
            <button
              key={kpi.id}
              type="button"
              ...
              className={cn(
                ...
                active ? "border-emerald-500/50 bg-emerald-500/10" : "border-zinc-700 bg-zinc-950/50 hover:bg-zinc-900",
              )}
            >
      ...
      {/* Radar + intelligence */}
      <div className="grid min-h-0 gap-3 lg:grid-cols-[minmax(280px,420px)_1fr]">
