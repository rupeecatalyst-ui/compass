...
/**
 * CO-ARCH-003 Phase 2B Sprint 2 / CO-BUG-008 — Enterprise Lender Search for Deal Workspace.
 * Auto-browses eligible active lenders on open; excludes existing Opportunity Deals by lenderId.
 */
...
import {
  ...
  programSupportsProduct,
  resolveProductLibraryCode,
} from "@/lib/deal-workspace/product-lender-eligibility";
import {
...
export function EnterpriseLenderSearch({
  ...
  onSelect,
  requireProgram = true,
  /** When true (default), show eligible lenders immediately without requiring a keystroke. */
  autoBrowse = true,
  className,
}: {
  ...
  const resolvedProduct = resolveProductLibraryCode({
    productCode,
    productLabel,
    loanProduct,
  });
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      void (async () => {
        ...
        try {
          const q = query.trim();
          const primary = await searchActiveLenders({
            search: q || undefined,
            pageSize: 120,
          });
          let merged = primary;
          if (q.length >= 2) {
            const productHit = resolveProductLibraryCode({
              productLabel: q,
              loanProduct: q,
            });
            if (productHit || primary.length < 8) {
              const broad = await searchActiveLenders({ pageSize: 120 });
              const byId = new Map(primary.map((l) => [l.id, l]));
  ...
  const eligible = useMemo(() => {
    ...
    return activePool.filter((l) => {
      if (!lenderSupportsProduct(l.productsSupported, resolvedProduct)) return false;
      if (!q) return true;
      const hay = [
        l.displayName,
        l.label,
  ...
  }, [activePool, resolvedProduct, query]);

  const displayLenders = eligible;

  const allEligibleAlreadyIdentified =
    !loading &&
    !error &&
    lenders.length > 0 &&
    activePool.length === 0 &&
    ...
    void (async () => {
      try {
        ...
        const filtered = items.filter((p) =>
          programSupportsProduct(p, resolvedProduct),
        );
        setPrograms(filtered.length ? filtered : items);
      } catch {
  ...
  }, [selectedLender?.id, resolvedProduct]);
  ...
  const emptyMessage = allEligibleAlreadyIdentified
    ? "All eligible lenders for this product have already been identified."
    : noEligibleRemain
      ? "No eligible active lenders match this product filter."
      : "No eligible active lenders found.";
      ...
      <div>
        ...
        <div className="relative mt-1">
          ...
          <Input
            ...
            placeholder="Search by lender name, code, or product…"
            value={query}
            onChange={(e) => {
        ...
        {resolvedProduct ? (
          <p className="mt-1 text-[10px] text-muted-foreground">
            Showing active lenders for{" "}
            <span className="font-medium text-foreground">{resolvedProduct}</span>
            {exclude.size > 0
          ...
          <p className="mt-1 text-[10px] text-muted-foreground">
            Active lenders only. Set Deal product to enable Product Library filtering.
          </p>
        )}
      </div>
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block text-[10px] text-muted-foreground">
                          ...
                          {l.institutionCategory
                            ? ` · ${l.institutionCategory}`
                            : ""}
                          {l.productsSupported?.length
                            ? ` · ${l.productsSupported.slice(0, 3).join(", ")}`
                            : ""}
                        </span>
                      </span>
                    </button>
        ...
        <div>
          ...
          {programsLoading ? (
            ...
            <p className="mt-1 text-[11px] text-amber-700 dark:text-amber-300">
              No active programs for this lender
              {resolvedProduct ? ` / product ${resolvedProduct}` : ""}. Select another
              lender or ask Administration to publish a program.
            </p>
          ) : (
            <select
          ...
          <p className="mt-1 text-[10px] text-muted-foreground">
            Programs refresh automatically when the lender changes. Invalid combinations
            are not offered.
          </p>
        </div>
      ) : null}
