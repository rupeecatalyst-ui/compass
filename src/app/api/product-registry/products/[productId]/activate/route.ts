...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await productRegistryService.activateProduct(
      productId,
      actor.userId,
    ...
    const message = err instanceof Error ? err.message : "Failed to activate product";
    if (message.includes("not found")) return notFound("Product not found");
    return errorResponse(400, "PRODUCT_ACTIVATE_FAILED", message);
  }
}
