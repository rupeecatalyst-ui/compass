...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await productRegistryService.deactivateCategory(
      categoryId,
      actor.userId,
    ...
    const message = err instanceof Error ? err.message : "Failed to deactivate product category";
    if (message.includes("not found")) return notFound("Product category not found");
    return errorResponse(400, "PRODUCT_CATEGORY_DEACTIVATE_FAILED", message);
  }
}
