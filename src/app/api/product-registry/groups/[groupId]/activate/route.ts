...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await productRegistryService.activateGroup(
      groupId,
      actor.userId,
    ...
    const message = err instanceof Error ? err.message : "Failed to activate product group";
    return errorResponse(400, "PRODUCT_GROUP_ACTIVATE_FAILED", message);
  }
}
