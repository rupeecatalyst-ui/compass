/**
 * @deprecated Alias — prefer `/api/invoice-parties/[partyId]`.
 */
import {
  errorResponse,
  fromAuthError,
...
type Ctx = { params: Promise<{ payeeId: string }> };
...
export async function GET(request: Request, context: Ctx) {
  try {
    ...
    const row = await invoicePartyService.get(payeeId);
...
export async function PATCH(request: Request, context: Ctx) {
  try {
    ...
    const row = await invoicePartyService.update(payeeId, body, actor.userId);
    return successResponse(row);
  } catch (err) {
    if (typeof err === "object" && err && "status" in err && "body" in err) {
      return fromAuthError(err as { status: number; body: ApiResponse<unknown> });
