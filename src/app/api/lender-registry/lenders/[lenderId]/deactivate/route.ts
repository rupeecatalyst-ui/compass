...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const updated = await lenderRegistryService.deactivateLender(
    ...
    const message = err instanceof Error ? err.message : "Failed to deactivate lender";

    if (message.includes("not found")) return notFound("Lender not found");

    return errorResponse(400, "LENDER_DEACTIVATE_FAILED", message);
