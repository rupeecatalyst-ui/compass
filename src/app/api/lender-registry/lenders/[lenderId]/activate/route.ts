...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const updated = await lenderRegistryService.activateLender(
    ...
    const message = err instanceof Error ? err.message : "Failed to activate lender";

    if (message.includes("not found")) return notFound("Lender not found");

    return errorResponse(400, "LENDER_ACTIVATE_FAILED", message);
