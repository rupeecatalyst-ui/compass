...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const updated = await lenderRegistryService.deactivateProgram(
    ...
    const message = err instanceof Error ? err.message : "Failed to deactivate lender program";

    if (message.includes("not found")) return notFound("Lender program not found");

    return errorResponse(400, "LENDER_PROGRAM_DEACTIVATE_FAILED", message);
