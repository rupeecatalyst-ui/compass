...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const updated = await lenderRegistryService.deactivateCategory(
    ...
    const message = err instanceof Error ? err.message : "Failed to deactivate lender category";

    if (message.includes("not found")) return notFound("Lender category not found");

    return errorResponse(400, "LENDER_CATEGORY_DEACTIVATE_FAILED", message);
