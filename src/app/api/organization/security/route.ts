...
import type { OrganizationSecurityConfigPatch } from "@/types/enterprise-organization-workspace";
import {
...
export async function GET(request: Request) {
  try {
    ...
    const config = await organizationWorkspaceService.getOrBootstrapSecurityConfig(actor);
...
export async function PATCH(request: Request) {
  try {
    guardOrganizationWorkspacePrisma();
    const actor = await resolveOrganizationActor(request);
    const body = (await request.json()) as OrganizationSecurityConfigPatch;
    const config = await organizationWorkspaceService.updateSecurityConfig(body, actor);
    return successResponse(config);
  } catch (err) {
    if (typeof err === "object" && err !== null && "status" in err) {
      return fromAuthError(err as { status: number; body: never });
