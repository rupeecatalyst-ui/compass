...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await referenceMasterService.restore(
      masterId,
      actor.userId,
    ...
    const message = err instanceof Error ? err.message : "Failed to restore reference master";
    return errorResponse(400, "REF_MASTER_RESTORE_FAILED", message);
  }
}
