...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await referenceMasterService.deactivate(
      masterId,
      actor.userId,
    ...
    const message = err instanceof Error ? err.message : "Failed to deactivate reference master";
    return errorResponse(400, "REF_MASTER_DEACTIVATE_FAILED", message);
  }
}
