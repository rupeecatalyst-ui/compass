...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await documentRegistryService.activateDefinition(
      definitionId,
      actor.userId,
    ...
    const message = err instanceof Error ? err.message : "Failed to activate document definition";
    if (message.includes("not found")) return notFound("Document definition not found");
    return errorResponse(400, "DOCUMENT_DEFINITION_ACTIVATE_FAILED", message);
  }
}
