...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await documentRegistryService.activateType(
      typeId,
      actor.userId,
    ...
    const message = err instanceof Error ? err.message : "Failed to activate document type";
    if (message.includes("not found")) return notFound("Document type not found");
    return errorResponse(400, "DOCUMENT_TYPE_ACTIVATE_FAILED", message);
  }
}
