...
import {
  getProductConversationNextStep,
  PRODUCT_CONVERSATION_ACKS,
  type ProductConversationAnswers,
  type ProductConversationConfig,
  type ProductConversationStep,
} from "@/config/product-conversation-types";
...
function renderStepInput(
  step: ProductConversationStep,
  drafts: { currency: string; city: string },
  setDrafts: React.Dispatch<React.SetStateAction<{ currency: string; city: string }>>,
  onContinue: (value: string) => void,
  filteredCities: readonly string[],
) {
  if (step.type === "options") {
    return <OptionButtons options={step.options} onSelect={onContinue} />;
  }
      ...
      <div className="space-y-4">
        <label className="relative block">
          ...
          <input
            ...
            onChange={(e) => setDrafts((d) => ({ ...d, currency: formatInrInput(e.target.value) }))}
            onKeyDown={(e) => {
              if (e.key === "Enter" && drafts.currency) onContinue(drafts.currency);
            }}
        ...
        <Button size="lg" className="h-12" disabled={!drafts.currency} onClick={() => onContinue(drafts.currency)}>
          Continue
    ...
    return (
      <div className="space-y-3">
        <label className="relative block">
          ...
          <input
            value={drafts.city}
            onChange={(e) => setDrafts((d) => ({ ...d, city: e.target.value }))}
            placeholder={step.placeholder}
  ...
  if (step.type === "reveal") {
    return (
      <Button size="lg" className="h-12" onClick={() => onContinue("reveal")}>
        {step.cta}
        <ArrowRight className="h-4 w-4" />
      </Button>
    );
...
interface ProductConversationSectionProps {
  conversation: ProductConversationConfig;
}

export function ProductConversationSection({ conversation }: ProductConversationSectionProps) {
  const reduceMotion = useReducedMotion();
  const [phase, setPhase] = useState<Phase>("welcome");
  const [answers, setAnswers] = useState<ProductConversationAnswers>({});
  const [stepId, setStepId] = useState<string | null>(conversation.steps[0]?.id ?? null);
  const [ackText, setAckText] = useState<(typeof PRODUCT_CONVERSATION_ACKS)[number]>(PRODUCT_CONVERSATION_ACKS[0]);
  const [ackCount, setAckCount] = useState(0);
  const [analysisStep, setAnalysisStep] = useState(0);
  const pendingAnswersRef = useRef<ProductConversationAnswers>({});
  const [drafts, setDrafts] = useState({ currency: "", city: "" });

  const currentStep = conversation.steps.find((s) => s.id === stepId);

  const filteredCities = useMemo(() => {
    const q = drafts.city.trim().toLowerCase();
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const next = getProductConversationNextStep(conversation.steps, pendingAnswersRef.current);
      setStepId(next);
      setPhase("chat");
    }, wait);
    return () => window.clearTimeout(timer);
  }, [phase, reduceMotion, conversation.steps]);
  ...
  useEffect(() => {
    ...
    const timers: number[] = [];
    conversation.analysis.steps.forEach((_, index) => {
      timers.push(window.setTimeout(() => setAnalysisStep(index + 1), (index + 1) * stepMs));
    });
    timers.push(
      window.setTimeout(
        () => setPhase("result"),
        (conversation.analysis.steps.length + 1) * finishMs,
      ),
    );
    return () => timers.forEach((id) => window.clearTimeout(id));
  }, [phase, reduceMotion, conversation.analysis.steps]);

  const showAckThenContinue = (patch: ProductConversationAnswers) => {
    const nextAnswers = { ...answers, ...patch };
    pendingAnswersRef.current = nextAnswers;
    setAnswers(nextAnswers);
    setAckText(PRODUCT_CONVERSATION_ACKS[ackCount % PRODUCT_CONVERSATION_ACKS.length]);
    setAckCount((c) => c + 1);
    setPhase("ack");
  };
  ...
  const restart = () => {
    ...
    setStepId(conversation.steps[0]?.id ?? null);
    setAckCount(0);
    setAnalysisStep(0);
    setDrafts({ currency: "", city: "" });
  };

  const handleStepContinue = (value: string) => {
    if (!currentStep) return;
    if (currentStep.type === "reveal") {
      setPhase("analysis");
      return;
    }
    showAckThenContinue({ [currentStep.id]: value });
  };
  ...
  return (
    <SectionReveal id="advantage-conversation" spacing="related" className="relative scroll-mt-24">
      ...
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_55%_45%_at_50%_0%,rgba(255,255,255,0.04),transparent)]" />

      <div className="relative mx-auto max-w-3xl">
        {conversation.empathyLine ? (
          <p className="mb-6 text-center text-sm font-medium italic text-accent/90">{conversation.empathyLine}</p>
        ) : null}
        ...
        <div className="overflow-hidden rounded-3xl glass-panel p-5 sm:p-8">
          <AnimatePresence mode="wait" initial={false}>
            {phase === "welcome" ? (
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Sarathi</p>
                    <p className="mt-1 text-lg font-semibold">{conversation.welcome.greeting}</p>
                  </div>
                </div>
                <h2 className="text-2xl font-bold tracking-tight sm:text-3xl">{conversation.welcome.headline}</h2>
                <p className="text-sm leading-relaxed text-muted-foreground sm:text-base">
                  {conversation.welcome.body}
                </p>
                <p className="text-xs text-primary/90">{conversation.welcome.note}</p>
                <Button size="lg" className="h-12" onClick={() => setPhase("chat")}>
                  {conversation.welcome.cta}
                  <ArrowRight className="h-4 w-4" />
                </Button>
              </motion.div>
            ) : null}
            ...
            {phase === "chat" && currentStep && currentStep.type !== "reveal" ? (
              <ChatFrame key={currentStep.id} title={currentStep.prompt} helper={"helper" in currentStep ? currentStep.helper : undefined}>
                {renderStepInput(currentStep, drafts, setDrafts, handleStepContinue, filteredCities)}
              </ChatFrame>
            ) : null}

            {phase === "chat" && currentStep?.type === "reveal" ? (
              <ChatFrame key="reveal" title={currentStep.prompt} helper={currentStep.helper}>
                {renderStepInput(currentStep, drafts, setDrafts, handleStepContinue, filteredCities)}
              </ChatFrame>
            ) : null}
            ...
            {phase === "analysis" ? (
              ...
              >
                <div className="flex items-center gap-3">
                  <SarathiAvatar size="sm" />
                  <h2 className="text-2xl font-bold tracking-tight">{conversation.analysis.title}</h2>
                </div>
                <ul className="space-y-3">
                  {conversation.analysis.steps.map((item, index) => {
            ...
            {phase === "result" ? (
              ...
              >
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-bold tracking-tight sm:text-3xl">{conversation.result.title}</h2>
                </div>

                <div className="grid gap-3 sm:grid-cols-2">
                  {conversation.result.cards.map((card) => (
